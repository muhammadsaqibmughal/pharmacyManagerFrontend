...
const CounterDetail = () => {
  ...
  // ✅ use `name` param instead of `id`
  const { name } = useParams();

  // ✅ decode from URL and find counter by name
  const decodedName = decodeURIComponent(name);
  const counter = counterIndex.find(
    (c) => c.name.toLowerCase() === decodedName.toLowerCase()
  );

  const salesByThisCounter = dummySales.filter(
    (s) => s.counterName.toLowerCase() === decodedName.toLowerCase()
  );
  ...
  // ✅ filter by date (searchTerm is a date string)
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredItems.length / ITEM_PER_PAGE)
  );
    ...
    <div className="p-10">
      <div className="flex justify-between gap-2 items-center mb-2">
        <div className="rounded-full px-4 py-2 bg-[#4F7942]">
          <Link to="/pos/customer/counter" className="text-sm text-white">
            ← Back
          </Link>
        </div>
      </div>
